...
def _process_img_morph(bin_img, threshold=.5, scale=1):
    # changed to only accept a binary img as input
    bin_img = bin_img.astype(np.bool)
    assert (bin_img >= 0).all(), f'Negative value encountered in image! {np.unique(bin_img)}'
    assert bin_img.dtype == np.bool, f'Expected type np.bool, got {type(bin_img)} of dtype {bin_img.dtype}'
#     bin_img = img_as_bool(bin_img)
    skel, dist_map = morphology.medial_axis(bin_img, return_distance=True)
    return bin_img, bin_img, skel, dist_map
