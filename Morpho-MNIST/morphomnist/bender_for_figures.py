...
# fig, ((axA, axB, axC, axD), (axE, axF, axG, axH)) = plt.subplots(2, 4)
...
def remove_path(image, orig_skel, new_skel, debug=False):
    ...
    if debug:
        plt.subplot(1,2,1)
        plt.imshow(invert(image), cmap="gray")
        no_grid_no_axis()
        plt.subplot(1,2,2)
        plt.imshow(invert(imnew), cmap="gray")
...
def bend_skel_with_endpts(skel, end1, end2, dist_shift, fixed_angle_seed=None, debug=False):
    ...
    for angle in angles: # pick the good bend
        ...
        if debug:
            plt.subplot(1,2,1)
            plt.imshow(invert(skel), cmap="gray")
            plt.subplot(1,2,2)
            global stored_skel
            stored_skel = invert(skelnew)
            plt.imshow(invert(skelnew), cmap="gray")
...
#     end1, end2 = choose_endpts(skel, distance, start_endpoints=start_endpoints)
    ...
    if debug:
        plt.subplot(1,2,1)
        plt.imshow(invert(skel))
        plt.subplot(1,2,2)
        plt.imshow(invert(bent), cmap="gray")
        # plt.title("bend_skel")
        no_grid_no_axis()
        plt.show()
    return bent
...
def add_bend(image, orig_skel, bent_skel, skeletonizer, debug=False):
    ...
    old_frame = np.copy(imnew)
    print("Uniques (frame):  ", np.unique(old_frame))
...
#         plt.imshow(dummy_img)
        plt.subplot(1,2,1)
        plt.imshow(stored_skel, cmap="gray")
        pts = np.where(difference(orig_skel, bent_skel) > 0)
        print(pts)
#         for pt in zip(*pts):
#             print("point: ", pt)
#             plt.plot(pt[0],pt[1], c='r')
        plt.plot(pts[1], pts[0], c='r')
        no_grid_no_axis()
        plt.subplot(1,2,2)
        plt.imshow(stored_skel, cmap="gray")
        pts = np.where(difference(bent_skel, orig_skel) > 0)
        print(pts)
#         for pt in zip(*pts):
#             print("point: ", pt)
#             plt.plot(pt[0],pt[1], c='r')
...
#             plt.imshow(imnew)
    ...
    if debug:
        plt.subplot(1,2,2)
        print("Uniques: ", np.unique(imnew), imnew.max())
        #imnew[imnew[imnew == 1]] = 0
        imnew[imnew!=0] = 120
        imnew[old_frame]= 255
        plt.imshow(invert(imnew), cmap="gray")
        pts = np.where(added_region>0)
        plt.plot(pts[1], pts[0], c='r')
        
        
        pts = np.where(imnew > 0)
        #plt.plot(pts[1], pts[0], c='b')
...
def bend_image(image, distance, dist_shift, mid_points=None, skeletonizer=None, fixed_angle_seed=None, relative=True, new_bend=False, debug=False):
    ...
    if debug:
        ...
        #axA.imshow(image, cmap="gray")
        #axB.imshow(imnew, cmap="gray")
...
def bender(image, distance, dist_shift, mid_points=None, skeletonizer=None, new_bend=False, relative=True, angle_seed=None, debug=False):
    ...
    """
    new_image = bend_image(image, distance, dist_shift, mid_points=mid_points, skeletonizer="skeletonize", 
                           new_bend=new_bend,
                           relative=relative,
                           fixed_angle_seed=angle_seed, debug=True)
