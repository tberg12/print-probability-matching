...
def sanity_check_black_fg_bin_img(bin_img):
    n_whites = bin_img.sum() # bg
    H,W = bin_img.shape
    n_blacks = H*W - n_whites # fg
    assert n_whites > n_blacks
    
def merge_images_np(img1, img2, method, foreground_color):
    """
       Returns union or intersection of binary numpy arrays
    """
    assert set(np.unique(img1)) == set([True, False]) and set(np.unique(img2)) == set([True, False])
    assert foreground_color == "black"
    sanity_check_black_fg_bin_img(img1)
    sanity_check_black_fg_bin_img(img2)
    
    if method == "union":
        inv_pil_img1 = Image.fromarray(invert(img1))
        inv_pil_img2 = Image.fromarray(invert(img2))
        merged_inv_imgs = ImageChops.add(inv_pil_img1, inv_pil_img2)
        merged_imgs = invert(np.array(merged_inv_imgs))
    
    elif method == "intersection":
        merged_imgs = np.array(ImageChops.add(pil_img1, pil_img2))
    
    else:
        raise ValueError(f"Invalid argument value: {method}")
    
    assert set(np.unique(merged_imgs)) == set([True, False])
    return merged_imgs
...
def choose_endpts(skeleton, distance, start_endpoints=None):
    ...
    for attempt in range(N):
        ...
        costs = invert(skeleton).astype(float) + 1 # use neg costs?
...
def nearest_neighbor_distance(pt, pts):
    distances = []
    assert pts.shape[1] == pt.shape[0] == 2
    for pt2 in pts:
        dist = np.linalg.norm(pt-pt2, ord=2)
        distances.append(dist)
    return min(distances)
        
def approximate_covered_area(curve1, curve2):
    assert curve1.shape[1] == curve2.shape[1] == 2
    area_slices = []
    for pt in curve1:
        d = nearest_neighbor_distance(pt, curve2)
        area_slices.append(d)
    area = sum(area_slices)
    return area


def get_approximate_area(orig_skel, bent_skel):
    dummy_img = np.zeros((orig_skel.shape[0], orig_skel.shape[1]))
    dummy_img[np.where(difference(orig_skel, bent_skel) > 0)]=1
    curve1 = np.stack(np.where(dummy_img == 1)).T 
    
    dummy_img = np.zeros((orig_skel.shape[0], orig_skel.shape[1]))
    dummy_img[np.where(difference(bent_skel, orig_skel) > 0)]=1
    curve2 = np.stack(np.where(dummy_img == 1)).T
    
    approx_area = approximate_covered_area(curve1, curve2)
...
def add_bend(image, orig_skel, bent_skel, skeletonizer, debug=False):
    ...
    approx_area = get_approximate_area(orig_skel, bent_skel)
    if debug:
        plt.subplot(1,3,1)
        dummy_img = np.zeros((orig_skel.shape[0], orig_skel.shape[1]))
        dummy_img[np.where(difference(orig_skel, bent_skel) > 0)]=1
        print(np.where(dummy_img == 1)) 
        plt.imshow(dummy_img) # original
        plt.subplot(1,3,3)
        dummy_img[np.where(difference(bent_skel, orig_skel) > 0)]=1
        plt.imshow(dummy_img) # mixed
        
        plt.subplot(1,3,2)
        dummy_img = np.zeros((orig_skel.shape[0], orig_skel.shape[1]))
        dummy_img[np.where(difference(bent_skel, orig_skel) > 0)]=1
        print(np.where(dummy_img == 1)) 
        plt.imshow(dummy_img) # new
    ...
    result = {"bent_image": imnew, "approximate_area": approx_area}
    return result
...
def bend_image(image, distance, dist_shift, mid_points=None, skeletonizer=None, fixed_angle_seed=None, relative=True, new_bend=False, debug=False):
    ...
    if relative:
        ...
        if debug:
            ...
            print(f"Old shift: {dist_shift}, scaled shift: {new_shift}, image height: {img_h}")
    ...
    if not new_bend:
        result = add_bend(image, skel, bent_skel, skeletonizer=skeletonizer, debug=debug)
        imnew = result["bent_image"]
    ...
    return {"bent_image": imnew, "original_skeleton": skel, "bent_skeleton": bent_skel, "approximate_area":approx_area}

def bender(image, distance, dist_shift, mid_points=None, skeletonizer=None, new_bend=False, relative=True, angle_seed=None, return_skeleton_distance=False, debug=False):
    """
       Bend `image` between two random points of the characters separated by `distance`, with a bend depth of `dist_shift`
        `debug` - Plots inputs/outputs at various stages for debugging
        `return_skeleton_distance` - returns the L2 distance b/w original and new bend portion to determine quality of 
        `skeletonizer` - skeletonization method (should be either `medial_aixs` or `skeletonize`)
    """
    np.random.shuffle(mid_points)
    result = bend_image(image, distance, dist_shift, mid_points=mid_points, skeletonizer=skeletonizer, 
                           new_bend=new_bend,
                           relative=relative,
                           fixed_angle_seed=angle_seed, debug=debug)
    new_image, skel, bent_skel = result["bent_image"], result["original_skeleton"], result["bent_skeleton"]
    approx_area = result["approximate_area"]
    bin_image = new_image.astype(float) # invert method wants the inputs to be float array
    
    if return_skeleton_distance:
        skel_L2 = calculate_distances(skel, [bent_skel])[0]
        return {"binary_image": bin_image, "skeleton_L2": skel_L2, "approximate_area": approx_area}
    else:
        return {"binary_image": bin_image, "approximate_area": approx_area}
