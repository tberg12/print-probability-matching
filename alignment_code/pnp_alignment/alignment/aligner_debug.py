...
class mixture_model(nn.Module):
    def __init__(self, ypix, nclasses=1, max_off=5, initializer=None, npts=0, rot=True):
        super(mixture_model, self).__init__()
        self.ypix = ypix
        self.max_off = max_off
        self.nc = nclasses
        # self.mix_prob = Parameter(torch.ones(nclasses))
        ...
        if npts > 0:
            ...
            # self.offsets = nn.Embedding(npts, 2)
        self.padder = nn.ConstantPad2d(max_off, 100.0)
        if initializer is not None:
            self.base_img.weight = Parameter(initializer)
            # initializer = initializer[0].reshape(ypix*ypix).repeat(nclasses,1)
            print(initializer.size())
            # if(nclasses>1):
            #  for n in range(nclasses-1):
            #    initializer[n]= torch.randn(ypix*ypix)
            # initializer[1]= torch.randn(ypix*ypix)
            self.base_img.weight = Parameter(initializer)
    ...
    def map(
        ...
        if not (batch_mode):
            # this case brach is behind. I don't care about non-batch
            tanx = torch.tan(1.5 * shear[0])
            tany = torch.tan(1.0 * shear[1])
            # wx = 1.0 - (1.0-width_var[0])*0.25
            wx = width_var[0] / pr[0]
            wy = width_var[1] / pr[1]
            mean_x = (
                torch.Tensor([range(xsz)]).squeeze().to(device) - (offset[0] / fac_x)
            ) * wx  # range
            mean_y = (
                torch.Tensor([range(ysz)]).squeeze().to(device) - (offset[1] / fac_y)
            ) * wy  # range
        if batch_mode:
            # try:
            ...
            # wx = (1.0 - (1.0-width_var[:,0])*0.25).unsqueeze(1) #b,1
            wx = (width_var[:, 0] / pr[:, 0]).unsqueeze(1)  # b,1
            wy = (width_var[:, 1] / pr[:, 1]).unsqueeze(1)  # b,1
            # mean_x = (torch.Tensor([range(xsz)]).squeeze().to(device) - (offset[:,0].unsqueeze(1)))*wx #b,range
            # mean_y = (torch.Tensor([range(ysz)]).squeeze().to(device) - (offset[:,1].unsqueeze(1)))*wy #b,range
            mean_x = (
                torch.Tensor([range(-math.floor(xsz / 2.0), math.ceil(xsz / 2.0))])
                .squeeze()
                .to(device)
                - ((offset[:, 0] / fac_x).unsqueeze(1))
            ) * wx  # b,range
            mean_y = (
    ...
    def combine(self, heatmap, base_img, batch_mode=False):
        ...
        if batch_mode:
            b_inc = 1
        return (
            (heatmap * torch.transpose(base_img, -2, -1)).sum(3 + b_inc).sum(2 + b_inc)
        )

    def continuous_offset(self, img, img_id, pad_ratio, cls, batch_mode=False):
        max_w = 0.4  # 0.3,0.7
        max_rot = 7.0  # 3.0,5.0
        max_shear = 3.5  # 3.5, 5.0
        ...
        if not (batch_mode):
            ...
            basic_img = self.padder(
                self.base_img(
                    torch.Tensor([int(cls)]).to(dtype=torch.long, device=device)
                )
                .squeeze()
            ...
            offset = self.max_off * (
                torch.tanh(
                    self.offsets(
                        torch.Tensor([int(opt.nc) * int(img_id) + int(cls)]).to(
            ...
            offset = self.max_off * (
                torch.tanh(self.offsets((int(opt.nc) * img_id) + cls))
            )  # b,2
            width_var = 1.0 - max_w * (
                torch.tanh(self.width((int(opt.nc) * img_id) + cls))
            )
            rotn_var = max_rot * (
                torch.tanh(self.rotn((int(opt.nc) * img_id) + cls))
            )  # 3.0
            shear_var = max_shear * (
                torch.tanh(self.shear((int(opt.nc) * img_id) + cls))
            )  # 3.5
...
"""def build_data(dir="./translation", init_dir=None, cl_id=False, bookname="tif"):
  maxc = opt.data_size
  images=[]
  cl_ids= []
  fnames=[]
  ...
  for filename in os.listdir(dir):
    #print(filename, bookname)
    if ((filename.endswith(".tif")) and (bookname in filename)):
      #print(filename)
      try:
        imp=to_grayscale(Image.open(dir+filename))
      except:
        print("skipping the image: "+str(dir+filename))
      else:
        totcount += 1
        if (totcount>maxc):
          break
  ...
  for filename in os.listdir(dir):
    if ((filename.endswith(".tif")) and (bookname in filename)):
      #real_imp=Image.open(dir+filename)
      try:
        imp=to_grayscale(Image.open(dir+filename))
      except:
        print("skipping the image: "+str(dir+filename))
        ...
        #hfrac = 1.0/hfrac
        try:
          imp = binarizer(pad(imp,(l,t,r,b),fill=255, padding_mode='constant'))
          gray_imp = pad(imp,(l,t,r,b),fill=255, padding_mode='constant')
        except:
          print("skipping the image: "+str(dir+filename))
        #real_imp = pad(real_imp,(l,t,r,b),fill=255, padding_mode='constant')
        #imp=(Image.open(dir+filename))
        else:
          imp = ToTensor()(imp)
          imp = imp.squeeze()#.to(device)
          images.append(imp)
          gray_images.append(gray_imp)
          cl_ids.append(cl)
          pad_ratios.append([wfrac, hfrac])
          fnames.append(filename)
    elif ((filename.endswith("g")) and (opt.init_avg)):
      imp=to_grayscale(Image.open(dir+filename))
      #imp=(Image.open(dir+filename))
      imp = (ToTensor()(imp))#/65535.0
      #print(imp.size())
      #imp = imp.squeeze().reshape(opt.y_pix*opt.y_pix)
      F.interpolate(imp.squeeze(),(opt.y_pix, opt.y_pix), mode = 'bilinear')
      initializer =imp
  initializers=None

  if (opt.init_rand) and (init_dir is None) and (len(images) >= opt.nc):
    init_imps = sample(images,opt.nc)
    #init_imps = [images[0],images[1]]
    initializers = torch.stack([F.interpolate(imp.unsqueeze(0).unsqueeze(0),(opt.y_pix, opt.y_pix), mode = 'area').squeeze().reshape(opt.y_pix*opt.y_pix)\
     for imp in init_imps])
    initializers = 0.1 + initializers*0.8
    initializers = torch.log(initializers) - torch.log(1.0 - initializers)
  elif not(init_dir is None):
    inits=[]
    for filename in os.listdir(init_dir):
      if filename.endswith(".tif"):
        imp=to_grayscale(Image.open(init_dir+filename))
        width,height = imp.size
        if ((width>opt.y_pix) or (height > opt.y_pix)):
          maxres = max([width,height])
          ratio = opt.y_pix*1.0/maxres
    ...
    for filename in os.listdir(init_dir):
      if filename.endswith(".tif"):
        ...
        b = math.floor(hdiff/2.0)
        t = math.ceil(hdiff/2.0)
        imp = pad(imp,(l,t,r,b),fill=255, padding_mode='constant')
        imp = ToTensor()(imp)
        imp = imp.squeeze()
        inits.append(imp)
        initializers = torch.stack([F.interpolate(imp.unsqueeze(0).unsqueeze(0),(opt.y_pix, opt.y_pix), mode = 'area').squeeze().reshape(opt.y_pix*opt.y_pix)\
  ...
  #print(initializers.size())
  #initializer = init_imp.reshape(opt.y_pix*opt.y_pix)
  print(maxw,maxh)
  return images,initializers, cl_ids, fnames, pad_ratios , gray_images
"""
...
def reconstruct(
    ...
    # wx = (1.0 - (1.0-width[:,0])*0.25).unsqueeze(1)
    ...
    tanx = torch.tan(-1.5 * shear[:, 0]).unsqueeze(1)
    tany = torch.tan(-1.0 * shear[:, 1]).unsqueeze(1)
    # mean_x = (torch.Tensor([range(opt.y_pix)]).squeeze().to(device) + (offset[:,0].unsqueeze(1)))/(wx)#/fac_x
    # mean_y = (torch.Tensor([range(opt.y_pix)]).squeeze().to(device) + (offset[:,1].unsqueeze(1)))/(wy)#/fac_y
    # mean_x = ((torch.Tensor([range(opt.y_pix)]).squeeze().to(device))/(wx) + (offset[:,0].unsqueeze(1)))#/fac_x
    # mean_y = ((torch.Tensor([range(opt.y_pix)]).squeeze().to(device))/(wy) + (offset[:,1].unsqueeze(1)))#/fac_y
    mean_x = (
    ...
    # dist_x = ((torch.Tensor([range(-(math.ceil(opt.max_off/fac_x)),xsz+math.ceil(opt.max_off/fac_x))]).to(device))).unsqueeze(0+b_inc)
    # dist_y = ((torch.Tensor([range(-(math.ceil(opt.max_off/fac_y)),ysz+math.ceil(opt.max_off/fac_y))]).to(device))).unsqueeze(0+b_inc)
    dist_x = (
        (
            torch.Tensor(
                [
...
def main():
    ...
    model = mixture_model(
        opt.y_pix, opt.nc, opt.max_off, initializer, npts, rot=not (opt.no_rotn)
    )
    ...
    optim_adj = build_optim(
        ...
        + [p for p in model.shear.parameters()],
