...
gathering_map = pl.read_excel("spinoza_map_w_printer_nikolai.xlsx")
print(all_sample.select(pl.col("book_name").unique()).to_series().to_list())

# Helper function to create background shading for sections
def create_section_background(gathering_map, split_ranges, book_key="spinozatheologicalpolitical1689"):
...
# Find letters that appear in non-spinoza books
non_spinoza_letters = set(
    all_sample
    .filter(~pl.col("filename").str.contains("theological"))
    ["letter_with_class"]
    .unique()
    .to_list()
)

print(f"Found {len(non_spinoza_letters)} letter types in non-spinoza books")

# Print book names found in the dataset
all_book_names = set(all_sample["book_name"].unique().to_list())
spinoza_book_names = set(
    all_sample
    .filter(pl.col("filename").str.contains("theological"))
    ["book_name"]
    .unique()
...
non_spinoza_book_names = all_book_names - spinoza_book_names

print(f"All book names found: {sorted(list(all_book_names))}")
print(f"Spinoza book names: {sorted(list(spinoza_book_names))}")
print(f"Non-spinoza book names: {sorted(list(non_spinoza_book_names))}")
...
full_sample = (
    ...
    .with_columns(
        ...
        appears_in_other_books=pl.col("letter_with_class").is_in(list(non_spinoza_letters))
    )
)
full_sample = full_sample.sort("letter_with_class")

# Create section background data for shading
section_background = create_section_background(gathering_map, SPLIT_RANGES, "spinozatheologicalpolitical1689")
print(f"Created section background data with {len(section_background) if section_background is not None else 0} rows")

# Create a mapping of letter_with_class to non-spinoza books where they appear
...
for letter in full_sample["letter_with_class"].unique().to_list():
    other_books = set(
        ...
        .filter(
            ...
            (~pl.col("filename").str.contains("theological"))
        )
        ["book_name"]
        .unique()
        .to_list()
    ...
    if other_books:
        # Join multiple books with " + " if letter appears in multiple non-spinoza books
        letter_to_other_books[letter] = " + ".join(sorted(list(other_books)))
    else:
        letter_to_other_books[letter] = "spinoza only"
...
spinoza_letters = set(full_sample["letter_with_class"].unique().to_list())
cross_book_letters = spinoza_letters.intersection(non_spinoza_letters)
spinoza_only_letters = spinoza_letters - non_spinoza_letters

print(f"Letters in spinoza: {len(spinoza_letters)}")
print(f"Letters that also appear in other books: {len(cross_book_letters)}")
print(f"Letters only in spinoza: {len(spinoza_only_letters)}")
print(f"Cross-book letter examples: {sorted(list(cross_book_letters))[:10]}")
print(f"Spinoza-only letter examples: {sorted(list(spinoza_only_letters))[:10]}")
print()
...
cross_book_heatmap = alt.Chart(full_sample.filter(pl.col("appears_in_other_books"))).mark_rect(height=8).encode(
    ...
    title="Letters that appear in both spinoza and other books",
    height=alt.Step(12),
    width=800
).resolve_scale(
    color='independent'
...
if section_background is not None:
    ...
    background_layer = alt.Chart(section_background).mark_rect().encode(
        x=alt.X("gathering", sort=all_gatherings_sorted),
        color=alt.Color("section:N", 
            scale=alt.Scale(scheme="pastel1"),
            legend=alt.Legend(title="Book Section")
        ),
    ...
    cross_book_heatmap_shaded = alt.layer(
        ...
        title="Letters that appear in both spinoza and other books (with section shading)",
        height=alt.Step(12),
        width=800
    ).resolve_scale(
        color='independent'
...
chunk_cross_book_heatmap = alt.Chart(
    ...
    title="Letters from chunk files that appear in both spinoza and other books",
    height=alt.Step(12),
    width=800
).resolve_scale(
    color='independent'
...
no_chunk_cross_book_heatmap = alt.Chart(
    ...
    title="Letters from non-chunk files that appear in both spinoza and other books",
    height=alt.Step(12),
    width=800
).resolve_scale(
    color='independent'
...
clustered_cross_book_heatmap = alt.Chart(
    ...
    title="Letters that appear in both spinoza and other books (by cluster)",
    height=alt.Step(12),
    width=400
).resolve_scale(
    color='independent'
...
clustered_chunk_cross_book_heatmap = alt.Chart(
    ...
    title="Letters from chunk files that appear in both spinoza and other books (by cluster)",
    height=alt.Step(12),
    width=400
).resolve_scale(
    color='independent'
...
clustered_no_chunk_cross_book_heatmap = alt.Chart(
    ...
    title="Letters from non-chunk files that appear in both spinoza and other books (by cluster)",
    height=alt.Step(12),
    width=400
).resolve_scale(
    color='independent'
...
# Get the original gathering order from full_sample to preserve in aggregated plots
# Use all_gatherings_sorted to ensure consistent custom ordering across all plots
...
agg_cross_book_heatmap = alt.Chart(expanded_all).mark_rect(height=8).encode(
    ...
    title="Damaged type presence in other books across spinoza gatherings",
    height=alt.Step(15),
    width=800
).resolve_scale(
    color='independent'
...
agg_chunk_cross_book_heatmap = alt.Chart(expanded_chunk).mark_rect(height=8).encode(
    ...
    title="Chunk damaged type presence in other books across spinoza gatherings",
    height=alt.Step(15),
    width=800
).resolve_scale(
    color='independent'
...
agg_no_chunk_cross_book_heatmap = alt.Chart(expanded_no_chunk).mark_rect(height=8).encode(
    ...
    title="Non-chunk damaged type presence in other books across spinoza gatherings",
    height=alt.Step(15),
    width=800
).resolve_scale(
    color='independent'
...
if section_background is not None:
    ...
    agg_no_chunk_cross_book_heatmap_shaded = alt.layer(
        ...
        title="Non-chunk damaged type presence in other books across spinoza gatherings (with section shading)",
        height=alt.Step(15),
        width=800
    ).resolve_scale(
        color='independent'
...
agg_clustered_cross_book_heatmap = alt.Chart(expanded_clustered_all).mark_rect(height=8).encode(
    ...
    title="Damaged type presence in other books across spinoza gatherings (by cluster)",
    height=alt.Step(15),
    width=400
).resolve_scale(
    color='independent'
...
agg_clustered_chunk_cross_book_heatmap = alt.Chart(expanded_clustered_chunk).mark_rect(height=8).encode(
    ...
    title="Chunk damaged type presence in other books across spinoza gatherings (by cluster)",
    height=alt.Step(15),
    width=400
).resolve_scale(
    color='independent'
...
agg_clustered_no_chunk_cross_book_heatmap = alt.Chart(expanded_clustered_no_chunk).mark_rect(height=8).encode(
    ...
    title="Non-chunk damaged type presence in other books across spinoza gatherings (by cluster)",
    height=alt.Step(15),
    width=400
).resolve_scale(
    color='independent'
...
clustered_cross_book_heatmap.save("spinoza_clustered_crossbook_heatmap_latex_bigletter.png", scale_factor=2)
clustered_chunk_cross_book_heatmap.save("spinoza_clustered_chunk_crossbook_heatmap_latex_bigletter.png", scale_factor=2)
clustered_no_chunk_cross_book_heatmap.save("spinoza_clustered_nochunk_crossbook_heatmap_latex_bigletter.png", scale_factor=2)

# Save the new aggregated versions
agg_cross_book_heatmap.save("spinoza_agg_crossbook_heatmap_latex_bigletter.png", scale_factor=2)
agg_chunk_cross_book_heatmap.save("spinoza_agg_chunk_crossbook_heatmap_latex_bigletter.png", scale_factor=2)
agg_no_chunk_cross_book_heatmap.save("spinoza_agg_nochunk_crossbook_heatmap_latex_bigletter.png", scale_factor=2)
agg_clustered_cross_book_heatmap.save("spinoza_agg_clustered_crossbook_heatmap_latex_bigletter.png", scale_factor=2)
agg_clustered_chunk_cross_book_heatmap.save("spinoza_agg_clustered_chunk_crossbook_heatmap_latex_bigletter.png", scale_factor=2)
agg_clustered_no_chunk_cross_book_heatmap.save("spinoza_agg_clustered_nochunk_crossbook_heatmap_latex_bigletter.png", scale_factor=2)
...
heatmap_complete = alt.Chart(full_sample).mark_rect(height=5).encode(
    ...
    title="Letter heatmap with complete gathering domain (shows data gaps)",
    height=alt.Step(10),
    width=800
).resolve_scale(
    color='independent'
...
cross_book_heatmap_complete = alt.Chart(full_sample.filter(pl.col("appears_in_other_books"))).mark_rect(height=8).encode(
    ...
    title="Cross-book letters across all of spinoza gatherings",
    height=alt.Step(12),
    width=800
).resolve_scale(
    color='independent'
...
agg_cross_book_heatmap_complete = alt.Chart(expanded_all).mark_rect(height=8).encode(
    ...
    title="Damaged type presence across all of spinoza gatherings",
    height=alt.Step(15),
    width=800
).resolve_scale(
    color='independent'
...
agg_no_chunk_cross_book_heatmap_complete = alt.Chart(expanded_no_chunk).mark_rect(height=8).encode(
    ...
    title="Non-chunk damaged type presence across all of spinoza gatherings",
    height=alt.Step(15),
    width=800
).resolve_scale(
    color='independent'
...
# save fig
images_l.save("spinoza_letters_latex_bigletter.png", scale_factor=2)
heatmap.save("spinoza_heatmap_latex_bigletter.png", scale_factor=2)
cross_book_heatmap.save("spinoza_crossbook_heatmap_latex_bigletter.png", scale_factor=2)
if section_background is not None:
    cross_book_heatmap_shaded.save("spinoza_crossbook_heatmap_shaded_latex_bigletter.png", scale_factor=2)
chunk_cross_book_heatmap.save("spinoza_chunk_crossbook_heatmap_latex_bigletter.png", scale_factor=2)
no_chunk_cross_book_heatmap.save("spinoza_nochunk_crossbook_heatmap_latex_bigletter.png", scale_factor=2)
lower_triangle_heatmap.save("spinoza_adjacency_latex_bigletter.png", scale_factor=2)

# Save complete domain versions
heatmap_complete.save("spinoza_heatmap_complete_domain_latex_bigletter.png", scale_factor=2)
cross_book_heatmap_complete.save("spinoza_crossbook_complete_domain_latex_bigletter.png", scale_factor=2)
agg_cross_book_heatmap_complete.save("spinoza_agg_crossbook_complete_domain_latex_bigletter.png", scale_factor=2)
agg_no_chunk_cross_book_heatmap_complete.save("spinoza_agg_nochunk_crossbook_complete_domain_latex_bigletter.png", scale_factor=2)

# Save shaded versions
if section_background is not None:
    agg_no_chunk_cross_book_heatmap_shaded.save("spinoza_agg_nochunk_crossbook_shaded_latex_bigletter.png", scale_factor=2)
...
if section_background is not None:
    print("Saved shaded versions showing section splits")