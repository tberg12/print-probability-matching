...
gathering_map = pl.read_excel("locke_two_treatises_map_nikolai.xlsx")

# Helper function to create background shading for sections
def create_section_background(gathering_map, split_ranges, book_key="twotreatisesofgov1690"):
...
# Find letters that appear in non-locke books
non_locke_letters = set(
    all_sample
    .filter(~pl.col("filename").str.contains("treatises"))
    ["letter_with_class"]
    .unique()
    .to_list()
)

print(f"Found {len(non_locke_letters)} letter types in non-locke books")

# Print book names found in the dataset
all_book_names = set(all_sample["book_name"].unique().to_list())
locke_book_names = set(
    all_sample
    .filter(pl.col("filename").str.contains("treatises"))
    ["book_name"]
    .unique()
...
non_locke_book_names = all_book_names - locke_book_names

print(f"All book names found: {sorted(list(all_book_names))}")
print(f"locke book names: {sorted(list(locke_book_names))}")
print(f"Non-locke book names: {sorted(list(non_locke_book_names))}")
...
full_sample = (
    ...
    .with_columns(
        ...
        appears_in_other_books=pl.col("letter_with_class").is_in(list(non_locke_letters))
    )
)
...
# Create a mapping of letter_with_class to non-locke books where they appear
...
for letter in full_sample["letter_with_class"].unique().to_list():
    other_books = set(
        ...
        .filter(
            ...
            (~pl.col("filename").str.contains("treatises"))
        )
        ["book_name"]
        .unique()
        .to_list()
    ...
    if other_books:
        # Join multiple books with " + " if letter appears in multiple non-locke books
        letter_to_other_books[letter] = " + ".join(sorted(list(other_books)))
    else:
        letter_to_other_books[letter] = "locke only"
...
locke_letters = set(full_sample["letter_with_class"].unique().to_list())
cross_book_letters = locke_letters.intersection(non_locke_letters)
locke_only_letters = locke_letters - non_locke_letters

print(f"Letters in locke: {len(locke_letters)}")
print(f"Letters that also appear in other books: {len(cross_book_letters)}")
print(f"Letters only in locke: {len(locke_only_letters)}")
print(f"Cross-book letter examples: {sorted(list(cross_book_letters))[:10]}")
print(f"locke-only letter examples: {sorted(list(locke_only_letters))[:10]}")
print()
...
cross_book_heatmap = alt.Chart(full_sample.filter(pl.col("appears_in_other_books"))).mark_rect(height=8).encode(
    ...
    title="Letters that appear in both locke and other books",
    height=alt.Step(12),
    width=800
).resolve_scale(
    color='independent'
...
section_background = create_section_background(gathering_map, SPLIT_RANGES, "twotreatisesofgov1690")
...
if section_background is not None:
    ...
    cross_book_heatmap_shaded = alt.layer(
        ...
        title="Letters that appear in both locke and other books (with section shading)",
        height=alt.Step(12),
        width=800
    ).resolve_scale(
        color='independent'
...
chunk_cross_book_heatmap = alt.Chart(
    ...
    title="Letters from chunk files that appear in both locke and other books",
    height=alt.Step(12),
    width=800
).resolve_scale(
    color='independent'
...
no_chunk_cross_book_heatmap = alt.Chart(
    ...
    title="Letters from non-chunk files that appear in both locke and other books",
    height=alt.Step(12),
    width=800
).resolve_scale(
    color='independent'
...
clustered_cross_book_heatmap = alt.Chart(
    ...
    title="Letters that appear in both locke and other books (by cluster)",
    height=alt.Step(12),
    width=400
).resolve_scale(
    color='independent'
...
clustered_chunk_cross_book_heatmap = alt.Chart(
    ...
    title="Letters from chunk files that appear in both locke and other books (by cluster)",
    height=alt.Step(12),
    width=400
).resolve_scale(
    color='independent'
...
clustered_no_chunk_cross_book_heatmap = alt.Chart(
    ...
    title="Letters from non-chunk files that appear in both locke and other books (by cluster)",
    height=alt.Step(12),
    width=400
).resolve_scale(
    color='independent'
...
agg_cross_book_heatmap = alt.Chart(expanded_all).mark_rect(height=8).encode(
    ...
    title="Damaged type presence in other books across locke gatherings",
    height=alt.Step(15),
    width=800
).resolve_scale(
    color='independent'
...
agg_chunk_cross_book_heatmap = alt.Chart(expanded_chunk).mark_rect(height=8).encode(
    ...
    title="Chunk damaged type presence in other books across locke gatherings",
    height=alt.Step(15),
    width=800
).resolve_scale(
    color='independent'
...
agg_no_chunk_cross_book_heatmap = alt.Chart(expanded_no_chunk).mark_rect(height=8).encode(
    ...
    title="Non-chunk damaged type presence in other books across locke gatherings",
    height=alt.Step(15),
    width=800
).resolve_scale(
    color='independent'
...
if section_background is not None:
    ...
    agg_no_chunk_cross_book_heatmap_shaded = alt.layer(
        ...
        title="Non-chunk damaged type presence in other books across locke gatherings (with section shading)",
        height=alt.Step(15),
        width=800
    ).resolve_scale(
        color='independent'
...
agg_clustered_cross_book_heatmap = alt.Chart(expanded_clustered_all).mark_rect(height=8).encode(
    ...
    title="Damaged type presence in other books across locke gatherings (by cluster)",
    height=alt.Step(15),
    width=400
).resolve_scale(
    color='independent'
...
agg_clustered_chunk_cross_book_heatmap = alt.Chart(expanded_clustered_chunk).mark_rect(height=8).encode(
    ...
    title="Chunk damaged type presence in other books across locke gatherings (by cluster)",
    height=alt.Step(15),
    width=400
).resolve_scale(
    color='independent'
...
agg_clustered_no_chunk_cross_book_heatmap = alt.Chart(expanded_clustered_no_chunk).mark_rect(height=8).encode(
    ...
    title="Non-chunk damaged type presence in other books across locke gatherings (by cluster)",
    height=alt.Step(15),
    width=400
).resolve_scale(
    color='independent'
...
# clustered_cross_book_heatmap.save("locke_clustered_crossbook_heatmap_latex_bigletter.png", scale_factor=2)
# clustered_chunk_cross_book_heatmap.save("locke_clustered_chunk_crossbook_heatmap_latex_bigletter.png", scale_factor=2)
clustered_no_chunk_cross_book_heatmap.save("locke_clustered_nochunk_crossbook_heatmap_latex_bigletter.png", scale_factor=2)

# Save the new aggregated versions
# agg_cross_book_heatmap.save("locke_agg_crossbook_heatmap_latex_bigletter.png", scale_factor=2)
# agg_chunk_cross_book_heatmap.save("locke_agg_chunk_crossbook_heatmap_latex_bigletter.png", scale_factor=2)
agg_no_chunk_cross_book_heatmap.save("locke_agg_nochunk_crossbook_heatmap_latex_bigletter.png", scale_factor=2)
# agg_clustered_cross_book_heatmap.save("locke_agg_clustered_crossbook_heatmap_latex_bigletter.png", scale_factor=2)
# agg_clustered_chunk_cross_book_heatmap.save("locke_agg_clustered_chunk_crossbook_heatmap_latex_bigletter.png", scale_factor=2)
agg_clustered_no_chunk_cross_book_heatmap.save("locke_agg_clustered_nochunk_crossbook_heatmap_latex_bigletter.png", scale_factor=2)
...
heatmap_complete = alt.Chart(full_sample).mark_rect(height=5).encode(
    ...
    title="Letter heatmap across all of locke gatherings",
    height=alt.Step(10),
    width=800
).resolve_scale(
    color='independent'
...
cross_book_heatmap_complete = alt.Chart(full_sample.filter(pl.col("appears_in_other_books"))).mark_rect(height=8).encode(
    ...
    title="Cross-book letters across all of locke gatherings",
    height=alt.Step(12),
    width=800
).resolve_scale(
    color='independent'
...
agg_cross_book_heatmap_complete = alt.Chart(expanded_all).mark_rect(height=8).encode(
    ...
    title="Damaged type presence across all of locke gatherings",
    height=alt.Step(15),
    width=800
).resolve_scale(
    color='independent'
...
agg_no_chunk_cross_book_heatmap_complete = alt.Chart(expanded_no_chunk).mark_rect(height=8).encode(
    ...
    title="Non-chunk damaged type presence across all of locke gatherings",
    height=alt.Step(15),
    width=800
).resolve_scale(
    color='independent'
...
# save fig
images_l.save("locke_letters_latex_bigletter.png", scale_factor=2)
heatmap.save("locke_heatmap_latex_bigletter.png", scale_factor=2)
cross_book_heatmap.save("locke_crossbook_heatmap_latex_bigletter.png", scale_factor=2)
if section_background is not None:
    cross_book_heatmap_shaded.save("locke_crossbook_heatmap_shaded_latex_bigletter.png", scale_factor=2)
chunk_cross_book_heatmap.save("locke_chunk_crossbook_heatmap_latex_bigletter.png", scale_factor=2)
no_chunk_cross_book_heatmap.save("locke_nochunk_crossbook_heatmap_latex_bigletter.png", scale_factor=2)
lower_triangle_heatmap.save("locke_adjacency_latex_bigletter.png", scale_factor=2)

# Save complete domain versions
heatmap_complete.save("locke_heatmap_complete_domain_latex_bigletter.png", scale_factor=2)
cross_book_heatmap_complete.save("locke_crossbook_complete_domain_latex_bigletter.png", scale_factor=2)
agg_cross_book_heatmap_complete.save("locke_agg_crossbook_complete_domain_latex_bigletter.png", scale_factor=2)
agg_no_chunk_cross_book_heatmap_complete.save("locke_agg_nochunk_crossbook_complete_domain_latex_bigletter.png", scale_factor=2)

# Save shaded versions
if section_background is not None:
    agg_no_chunk_cross_book_heatmap_shaded.save("locke_agg_nochunk_crossbook_shaded_latex_bigletter.png", scale_factor=2)
...
if section_background is not None:
    print("Saved shaded versions showing book section splits")